...
import { daos, daosForSelect } from 'application/constants';
...
const NFTsPageComponent: React.FC<NFTsPageComponentProps> = ({
  ...
  const isDraftBool = useMemo(
    () => (isMyNftsPage && (isAuthor || isModerator)
      ? NFTStatus === 'draft'
      : isDraft),
    [isMyNftsPage, isAuthor, isModerator, NFTStatus, isDraft],
  );
  ...
  const renderProfile = useCallback(() => {
    ...
    const emailClassName = isShowEmail ? styles.profileColText : styles.profileColText__email;
    if (isProfileLoading) {
      return (
        <Skeleton
        ...
        <div className={styles.headCol}>
          <div className={styles.profileImageBlock}>
            ...
            <div className={styles.profileUserTariffLevel}>
              {t(userTariffLevelMap?.[userTariffLevel?.foundLevel || 0])}
            </div>
          </div>
        ...
        <div className={styles.row}>
          ...
          <div className={styles.col}>
            <div className={styles.colSet}>
              ...
              {isMyNftsPage && (isModerator || isAuthor) && (
                <Filter
                  label={t('NFTStatus')}
                  value={NFTStatus}
