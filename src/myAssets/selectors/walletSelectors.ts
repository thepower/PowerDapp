...
export const getWalletInitialLastBlock = (state: RootState) =>
  state.wallet.initialLastBlock;
export const getWalletLastBlock = (state: RootState) => state.wallet.lastblk;
export const getWalletNativeTokensAmounts = (state: RootState) =>
  state.wallet.amounts;
export const getWalletNativeTokensAmountByID = (
  state: RootState,
  symbol: string
) => state.wallet.amounts?.[symbol];
