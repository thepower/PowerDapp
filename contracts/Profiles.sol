...
contract UserProfile {
  ...
  function _match(
    ...
    for (uint i = 0; i < flt.length; i++) {
      if (userProfile[user][flt[i].k].length != flt[i].v.length) return false;
...
contract Profiles is UserProfile, AccessControl {
  ...
  function _matchRoles(
    address user,
    bytes32[] memory roles
  ) internal view returns (bool) {
    for (uint i = 0; i < roles.length; i++) {
      if (!hasRole(roles[i], user)) return false;
    }
    return true;
  }

  function _matchNoRoles(
    address user,
    bytes32[] memory roles
  ) internal view returns (bool) {
    for (uint i = 0; i < roles.length; i++) {
  ...
  function setRoleAdmin(
    bytes32 role,
    bytes32 adminRole
  ) public onlyRole(ROOT_ADMIN_ROLE) {
